...
class Bert_Model(nn.Module):
    def __init__(self):
        ...
        self.Wq = nn.Parameter(torch.Tensor(768 // 2, 768))
        self.Wk = nn.Parameter(torch.Tensor(768 // 2, 768))
        self.Wv = nn.Parameter(torch.Tensor(768 // 2, 768))

        self.Wq1 = nn.Parameter(torch.Tensor(768 // 2, 768))
        self.Wk1 = nn.Parameter(torch.Tensor(768 // 2, 768))
        self.Wv1 = nn.Parameter(torch.Tensor(768 // 2, 768))
        # init parameter
        nn.init.uniform_(self.Wq, -0.1, 0.1)
        nn.init.uniform_(self.Wk, -0.1, 0.1)
        nn.init.uniform_(self.Wv, -0.1, 0.1)

        nn.init.uniform_(self.Wq1, -0.1, 0.1)
        nn.init.uniform_(self.Wk1, -0.1, 0.1)
        nn.init.uniform_(self.Wv1, -0.1, 0.1)

        self.fc_w = nn.Linear(768, 768)
        self.activate_w = nn.LeakyReLU()
    ...
    def self_attention(self, x, prior):
        ...
        # head2
        Q1 = torch.matmul(self.Wq1, x.permute(0, 2, 1))  # [batch, q_dim, seq_len]
        K1 = torch.matmul(self.Wk1, x.permute(0, 2, 1))  # [batch, k_dim, seq_len]
        v1 = torch.matmul(self.Wv1, x.permute(0, 2, 1))  # [batch, v_dim, seq_len]
        # [batch, seq_len, seq_len]
        att_score1 = torch.softmax(torch.matmul(K1.permute(0, 2, 1), Q) / math.sqrt(Q1.shape[1]), dim=2)
        att_score_add_prior1 = att_score1 + prior
        att_score_new1 = torch.softmax(att_score_add_prior1, dim=2)

        generate_seq1 = torch.matmul(att_score_new1, v1.permute(0, 2, 1))  # [batch_size, seq_len, embedding_dim]

        # concat and pass a linear linear
        generate_seq_concat = torch.cat((generate_seq, generate_seq1), -1)

        generate_seq_concat = self.fc_w(generate_seq_concat.reshape(-1, 768)).reshape(generate_seq.shape[0],
                                                                                      generate_seq.shape[1],
                                                                                      -1)

        return generate_seq_concat  # [batch_size, seq_len, embedding_dim]
